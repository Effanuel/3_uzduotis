...
template <typename T> T generateStudents(unsigned int n) {
	T Studentai;
	...
	for (int i = 0; i < n; ++i) {
		...
		Sukurti_studenta(Studentai, "Vardas" + std::to_string(i), "Pavarde" + std::to_string(i), temp_balai, distr(gen));
		temp_balai.clear();
	}
...
//	for (typename T::size_type it = 0; it != Studentai.size(); ++it) {
